...
The app integrates with the users' dedicated mailbox from which it reads travel booking emails (currently flight/train/bus, car rental, accommodation). These are passed to Nextcloud Assistant to classify and extract the relevant information such as dates, passengers, providers, etc. into a structured schema. The extracted bookings are stored in Nextcloud and can then be grouped into trips in the application UI. A mobile client is envisioned for the future.
...
* Currently there is 1:N relationship between email message and bookings. In other words, a later email will not update an existing booking (in most cases a "duplicate" should be detected and highlighted).
